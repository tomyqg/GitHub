...
void GL_DrawArc2(int x0, int y0, int rx, int ry, float a0, float a1);
...
static void _DrawArc2(int x0, int y0, int rx, int ry, float Angle0, float Angle1, int xMul, int yMul) {
  ...
  Sin0_HQ = GUI__SinHQ((long)(Angle0 * (float)1000)); 
  Sin1_HQ = GUI__SinHQ((long)(Angle1 * (float)1000)); 
  Cos0_HQ = GUI__CosHQ((long)(Angle0 * (float)1000)); 
  Cos1_HQ = GUI__CosHQ((long)(Angle1 * (float)1000)); 
  ri2     = (U32)(ri * ri);
  ro2     = (U32)(ro * ro);
...
DoFirst:
  ...
  if (a0 < 90) {
    ...
    aEnd = (a1 < 90) ? a1 : 90;
    _DrawArc2(x0, y0, rx, ry, a0, aEnd, 1, -1);
  }
  a1 -= 90;
  a0 -= 90;
/* Do second quadrant 90-180 degree */
  ...
  if (a0 < 90) {
    ...
    aEnd = (a1 < 90) ? a1 : 90;
    _DrawArc2(x0, y0, rx, ry, 90 - aEnd, 90 - a0, -1, -1);
  }
  a1 -= 90;
  a0 -= 90;
...
*       GUI_DrawArc
*/
void GUI_DrawArc2(int x0, int y0, int rx, int ry, float a0, float a1) {
  GUI_LOCK();
  #if (GUI_WINSUPPORT)
    WM_ADDORG(x0,y0);
    WM_ITERATE_START(NULL) {
  #endif
  GL_DrawArc2( x0, y0, rx, ry, a0, a1);
  #if (GUI_WINSUPPORT)
    } WM_ITERATE_END();
  #endif
  GUI_UNLOCK();
