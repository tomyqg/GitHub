...
** emWin V4.16 - Graphical user interface for embedded applications **
...
static int _GetImages(GUI_MEMDEV_Handle * phMem, GUI_MEMDEV_Handle * phMemNarrow, int MaxItems) {
  ...
  */
  for (i = 0; (i < GUI_COUNTOF(_apData)) && (i < MaxItems); i++) {
    ...
    *(phMem + i) = GUI_MEMDEV_CreateFixed(0, 0, Info.XSize, Info.YSize, GUI_MEMDEV_NOTRANS, GUI_MEMDEV_APILIST_32, &LCD_API_ColorConv_8888);
    if (*(phMem + i) == 0) {
      return 1;
    }
  ...
  */
  for (i = 0; i < 6; i++) {
    *(phMemNarrow + i) = GUI_MEMDEV_CreateFixed(0, 0, 60, 120, GUI_MEMDEV_NOTRANS, GUI_MEMDEV_APILIST_32, &LCD_API_ColorConv_8888);
    if (*(phMemNarrow + i) == 0) {
      return 1;
    }
  }
