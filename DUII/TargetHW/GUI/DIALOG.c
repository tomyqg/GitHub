...
void GUI_EndDialog(WM_HWIN hDialog, int r) {
  ...
  //
  // WM__DeleteSecure makes sure that the window will be deleted within
  // the next execution of WM_Exec instead of deleting it immediately.
  // Deleting immediately makes problems because further messages are
  // send to the window under certain circumstances.
  //
  WM__DeleteSecure(hDialog);
}
