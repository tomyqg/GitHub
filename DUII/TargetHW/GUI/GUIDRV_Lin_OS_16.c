...
*       Draw Bitmap 16 BPP
*/
static void _DrawBitLine16BPP_Swap(GUI_DEVICE * pDevice, int x, int y, U16 const GUI_UNI_PTR * pData, int ySize, int BytesPerLine) {
  DRIVER_CONTEXT * pContext;
  U32 Off;
  U16 * pDest;
  int WordsPerLine;
...
static void _DrawBitmap(GUI_DEVICE * pDevice, int x0, int y0,
  ...
  case 16:
    for (i = Diff; i < xSize; i++) {
      _DrawBitLine16BPP_Swap(pDevice, x0 + i, y0, (U16 *)(pData + (i << 1)), ySize, BytesPerLine);
    }
    break;
  }
}
