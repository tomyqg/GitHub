...
static int _DeleteWindows; // Is set to 1 if any window needs to be deleted within WM_Exec()
...
void WM__InvalidateParent(const GUI_RECT * pInvalidRect, WM_HWIN hParent, WM_HWIN hStop) {
  ...
  pWin = WM_HANDLE2PTR(hParent);
  Flags = (pWin->Status & WM_CF_STATIC) ? WM_SF_INVALID_DRAW : WM_SF_INVALID;
  WM__InvalidateRect(pInvalidRect, hParent, hStop, Flags);
}
...
*/
static void _Invalidate1Abs(WM_HWIN hWin, const GUI_RECT * pRect, U16 Flags) {
  GUI_RECT r;
  WM_Obj * pWin;
  int Status;
  ...
  if (WM__RectIsNZ(&r)) {
    ...
    #endif
    if (pWin->Status & (WM_SF_INVALID | WM_SF_INVALID_DRAW)) {
...
void WM__Invalidate1Abs(WM_HWIN hWin, const GUI_RECT * pRect) {
  _Invalidate1Abs(hWin, pRect, WM_SF_INVALID);
}
...
void WM__InvalidateDrawAndDescs(WM_HWIN hWin) {
  ...
  if (hWin) {
    ...
    if (pWin->hMem) {
      GUI_MEMDEV_Delete(pWin->hMem);
      pWin->hMem = 0;
    }
    for (hChild = WM_GetFirstChild(hWin); hChild; hChild = pChild->hNext) {
      WM__InvalidateDrawAndDescs(hChild);
...
*  Invalidate window and all descendants (children and grandchildren and ...
*/
void WM_InvalidateWindowAndDescsEx(WM_HWIN hWin, const GUI_RECT * pInvalidRect, U16 Flags) {
  ...
  if (hWin) {
    ...
    _Invalidate1Abs(hWin, &Rect, Flags);    /* Invalidate window itself */
    for (hChild = WM_GetFirstChild(hWin); hChild; hChild = pChild->hNext) {
      WM_InvalidateWindowAndDescsEx(hChild, &Rect, Flags);
      pChild = WM_H2P(hChild);
    }
  }
}
...
void WM__InvalidateRect(const GUI_RECT* pInvalidRect, WM_HWIN hParent, WM_HWIN hStop, U16 Flags) {
  ...
  if ((Status & (WM_SF_HASTRANS | WM_SF_CONST_OUTLINE)) == WM_SF_HASTRANS) {
    WM__InvalidateRect(&Rect, pParent->hParent, pParent->hNext, Flags);
  } else {
    _Invalidate1Abs(hParent, &Rect, Flags);
  }
  ...
  for (hi = pParent->hFirstChild; hi; hi = pi->hNext) {
    ...
    WM_InvalidateWindowAndDescsEx(hi, &Rect, Flags);
    pi = WM_H2P(hi);
  }
}
...
void WM__InvalidateRectEx(const GUI_RECT * pInvalidRect, WM_HWIN hParent, WM_HWIN hStop) {
  WM__InvalidateRect(pInvalidRect, hParent, hStop, WM_SF_INVALID);
}
...
void WM_InvalidateRect(WM_HWIN hWin, const GUI_RECT * pRect) {
  ...
  if (hWin) {
    ...
    if (Status & WM_SF_ISVIS) {
      r = pWin->Rect;
      if (Status & WM_CF_STATIC) {
        WM__InvalidateDrawAndDescs(hWin); /* Delete static memory devices */
      }
      if (pRect) {
        rPara = *pRect;
...
WM_HWIN WM_CreateWindowAsChild( int x0, int y0, int width, int height
                               ,WM_HWIN hParent, U32 Style, WM_CALLBACK * cb
                               ,int NumExtraBytes) {
  WM_Obj * pWin;
  WM_Obj * pParent;
  WM_HWIN hWin;
  ...
  if ((hWin = (WM_HWIN) GUI_ALLOC_AllocZero(NumExtraBytes + sizeof(WM_Obj))) == 0) {
    ...
    pWin->Status |= (Style & (U32)(WM_CF_SHOW |
                              WM_SF_MEMDEV |
...
void WM_DeleteWindow (WM_HWIN hWin) {
  ...
  if (WM__IsWindow(hWin)) {
    ...
    WM__DeleteAssocTimer(hWin);
    pWin = WM_H2P(hWin);
    /* Delete static memory device */
    #if (WM_SUPPORT_STATIC_MEMDEV && GUI_SUPPORT_MEMDEV)
      if (pWin->hMem) {
        GUI_MEMDEV_Delete(pWin->hMem);
      }
    #endif
...
int WM__InitIVRSearch(const GUI_RECT* pMaxRect) {
 ...
 /* When using callback mechanism, it is legal to reduce drawing
  ...
  if (GUI_Context.WM__pUserClipRect) {
    pWin  = pAWin;
    rUser = *(GUI_Context.WM__pUserClipRect);
    #if WM_SUPPORT_TRANSPARENCY
      if (WM__hATransWindow) {
...
void WM__Paint1(WM_HWIN hWin, WM_Obj * pWin) {
  ...
  if ((pWin->cb != NULL)  && (Status & WM_SF_ISVIS)) {
    ...
    if (Status & WM_SF_LATE_CLIP) {
      ...
      InvalidRect = pWin->InvalidRect;
      WM_ITERATE_START(&InvalidRect) {
        Msg.hWin   = hWin;
        Msg.MsgId  = WM_PAINT;
        Msg.Data.p = (GUI_RECT*)&InvalidRect;
        WM_SetDefault();
        WM__SendMessage(hWin, &Msg);
      } WM_ITERATE_END();
    }
...
static void _Paint1(WM_HWIN hWin, WM_Obj * pWin) {
  WM_tfPaint1Func pfPaint1Func;

  #if WM_SUPPORT_STATIC_MEMDEV
    pfPaint1Func = (pWin->Status & WM_CF_STATIC) ? GUI_MEMDEV_Paint1Static : WM__Paint1;
  #else
    pfPaint1Func = WM__Paint1;
  #endif
  pfPaint1Func(hWin, pWin);
}
...
*/
static void WM__PaintWinAndOverlays(WM_HWIN hWin) {
  WM_Obj * pWin;
...
static void _cbPaintMemDev(void * p) {
  ...
  WM__PaintWinAndOverlays(*(WM_HWIN *)p);
  /* Within the paint event the application is allowed to deal with memory devices.
     So the pointer(s) could be invalid after the last function call and needs
     to be restored.
  */
      ...
      if (WM__ClipAtParentBorders(&pWin->InvalidRect, hWin)) {
        WM_SelectWindow(hWin);
          ...
          if (pWin->Status & WM_SF_MEMDEV) {
            if (pWin->hParent == 0) {
              //
              // Desktop windows do not have transparency
              //
              Flags = GUI_MEMDEV_NOTRANS;
            } else {
              //
              // Translate WM_SF_HASTRANS to GUI_MEMDEV_HASTRANS or GUI_MEMDEV_NOTRANS
              //
              Flags = (pWin->Status & WM_SF_HASTRANS) ? GUI_MEMDEV_HASTRANS : GUI_MEMDEV_NOTRANS;
            }
            GUI_MEMDEV_Draw(&pWin->InvalidRect, _cbPaintMemDev, &hWin, 0, Flags);
          } else
        #endif
        {
          WM__PaintWinAndOverlays(hWin);
          Ret = 1;    // Something has been done
        }
      }
    }
    //
    // We purposely clear the invalid flag after painting so we can still query the invalid rectangle while painting
    //
    pWin = WM_H2P(hWin);
    pWin->Status &= ~(WM_SF_INVALID | WM_SF_INVALID_DRAW); // Clear invalid flag
...
static void _DrawNext(void) {
  ...
  GUI_COLOR ColorOld, BkColorOld;

  iWin = (_NextDrawWin == WM_HWIN_NULL) ? WM__FirstWin : _NextDrawWin;
  /* Save context */
  GUI_SaveContext(&ContextOld);
  ColorOld   = GUI_Context.Color;
  BkColorOld = GUI_Context.BkColor;
  ...
  /* Make sure alpha blending device settings matches color settings of context */
  GUI_SetColor(ColorOld);
  GUI_SetBkColor(BkColorOld);
  /* Restore context */
...
*       _DeleteDelayed
*/
static void _DeleteDelayed(void) {
  WM_HWIN  hWin, hWinDelete;
  WM_Obj * pWin;

  WM_LOCK();
  for (hWin = WM__FirstWin; hWin;) {
    pWin = WM_H2P(hWin);
    if (pWin->Status & WM_SF_DELETE) {
      hWinDelete = hWin;
      hWin = pWin->hNextLin;
      WM_DeleteWindow(hWinDelete);
    } else {
      hWin = pWin->hNextLin;
    }
  }
