; Listing generated by Microsoft (R) Optimizing Compiler Version 16.00.40219.01 

	TITLE	C:\DUII\Simulation\GUI\Widget\FRAMEWIN_ButtonMin.c
	.686P
	.XMM
	include listing.inc
	.model	flat

INCLUDELIB LIBCMT
INCLUDELIB OLDNAMES

PUBLIC	_FRAMEWIN_AddMinButton
EXTRN	_WM_SetCallback:PROC
EXTRN	_BUTTON_SetSelfDraw:PROC
EXTRN	_FRAMEWIN_AddButton:PROC
EXTRN	__RTC_CheckEsp:PROC
EXTRN	__RTC_Shutdown:PROC
EXTRN	__RTC_InitBase:PROC
;	COMDAT rtc$TMZ
; File c:\duii\simulation\gui\widget\framewin_buttonmin.c
rtc$TMZ	SEGMENT
__RTC_Shutdown.rtc$TMZ DD FLAT:__RTC_Shutdown
rtc$TMZ	ENDS
;	COMDAT rtc$IMZ
rtc$IMZ	SEGMENT
__RTC_InitBase.rtc$IMZ DD FLAT:__RTC_InitBase
; Function compile flags: /Odtp /RTCsu /ZI
rtc$IMZ	ENDS
;	COMDAT _FRAMEWIN_AddMinButton
_TEXT	SEGMENT
_hButton$ = -8						; size = 4
_hObj$ = 8						; size = 4
_Flags$ = 12						; size = 4
_Off$ = 16						; size = 4
_FRAMEWIN_AddMinButton PROC				; COMDAT
; Line 137
	push	ebp
	mov	ebp, esp
	sub	esp, 204				; 000000ccH
	push	ebx
	push	esi
	push	edi
	lea	edi, DWORD PTR [ebp-204]
	mov	ecx, 51					; 00000033H
	mov	eax, -858993460				; ccccccccH
	rep stosd
; Line 139
	push	8
	mov	eax, DWORD PTR _Off$[ebp]
	push	eax
	mov	ecx, DWORD PTR _Flags$[ebp]
	push	ecx
	mov	edx, DWORD PTR _hObj$[ebp]
	push	edx
	call	_FRAMEWIN_AddButton
	add	esp, 16					; 00000010H
	mov	DWORD PTR _hButton$[ebp], eax
; Line 140
	push	OFFSET __Draw
	push	0
	mov	eax, DWORD PTR _hButton$[ebp]
	push	eax
	call	_BUTTON_SetSelfDraw
	add	esp, 12					; 0000000cH
; Line 141
	push	OFFSET __cbMin
	mov	eax, DWORD PTR _hButton$[ebp]
	push	eax
	call	_WM_SetCallback
	add	esp, 8
; Line 142
	mov	eax, DWORD PTR _hButton$[ebp]
; Line 143
	pop	edi
	pop	esi
	pop	ebx
	add	esp, 204				; 000000ccH
	cmp	ebp, esp
	call	__RTC_CheckEsp
	mov	esp, ebp
	pop	ebp
	ret	0
_FRAMEWIN_AddMinButton ENDP
_TEXT	ENDS
EXTRN	_BUTTON_Callback:PROC
EXTRN	_FRAMEWIN_Minimize:PROC
EXTRN	_FRAMEWIN_Restore:PROC
EXTRN	_GUI_ALLOC_UnlockH:PROC
EXTRN	_FRAMEWIN_LockH:PROC
; Function compile flags: /Odtp /RTCsu /ZI
;	COMDAT __cbMin
_TEXT	SEGMENT
_Flags$13064 = -32					; size = 2
_pObj$13063 = -20					; size = 4
_hObj$13062 = -8					; size = 4
_pMsg$ = 8						; size = 4
__cbMin	PROC						; COMDAT
; Line 51
	push	ebp
	mov	ebp, esp
	sub	esp, 228				; 000000e4H
	push	ebx
	push	esi
	push	edi
	lea	edi, DWORD PTR [ebp-228]
	mov	ecx, 57					; 00000039H
	mov	eax, -858993460				; ccccccccH
	rep stosd
; Line 52
	mov	eax, DWORD PTR _pMsg$[ebp]
	cmp	DWORD PTR [eax], 39			; 00000027H
	jne	SHORT $LN3@cbMin
; Line 56
	mov	eax, DWORD PTR _pMsg$[ebp]
	mov	ecx, DWORD PTR [eax+8]
	mov	DWORD PTR _hObj$13062[ebp], ecx
; Line 57
	mov	eax, DWORD PTR _hObj$13062[ebp]
	push	eax
	call	_FRAMEWIN_LockH
	add	esp, 4
	mov	DWORD PTR _pObj$13063[ebp], eax
; Line 58
	mov	eax, DWORD PTR _pObj$13063[ebp]
	mov	cx, WORD PTR [eax+128]
	mov	WORD PTR _Flags$13064[ebp], cx
; Line 59
	call	_GUI_ALLOC_UnlockH
	mov	DWORD PTR _pObj$13063[ebp], 0
; Line 60
	movzx	eax, WORD PTR _Flags$13064[ebp]
	and	eax, 64					; 00000040H
	je	SHORT $LN2@cbMin
; Line 61
	mov	eax, DWORD PTR _hObj$13062[ebp]
	push	eax
	call	_FRAMEWIN_Restore
	add	esp, 4
; Line 62
	jmp	SHORT $LN1@cbMin
$LN2@cbMin:
; Line 63
	mov	eax, DWORD PTR _hObj$13062[ebp]
	push	eax
	call	_FRAMEWIN_Minimize
	add	esp, 4
$LN1@cbMin:
; Line 65
	jmp	SHORT $LN4@cbMin
$LN3@cbMin:
; Line 67
	mov	eax, DWORD PTR _pMsg$[ebp]
	push	eax
	call	_BUTTON_Callback
	add	esp, 4
$LN4@cbMin:
; Line 68
	pop	edi
	pop	esi
	pop	ebx
	add	esp, 228				; 000000e4H
	cmp	ebp, esp
	call	__RTC_CheckEsp
	mov	esp, ebp
	pop	ebp
	ret	0
__cbMin	ENDP
_TEXT	ENDS
EXTRN	_GUI_SetColor:PROC
EXTRN	_WM_GetParent:PROC
; Function compile flags: /Odtp /RTCsu /ZI
;	COMDAT __Draw
_TEXT	SEGMENT
_hFrame$ = -32						; size = 4
_Flags$ = -20						; size = 2
_pFrame$ = -8						; size = 4
_hObj$ = 8						; size = 4
__Draw	PROC						; COMDAT
; Line 110
	push	ebp
	mov	ebp, esp
	sub	esp, 228				; 000000e4H
	push	ebx
	push	esi
	push	edi
	lea	edi, DWORD PTR [ebp-228]
	mov	ecx, 57					; 00000039H
	mov	eax, -858993460				; ccccccccH
	rep stosd
; Line 115
	mov	eax, DWORD PTR _hObj$[ebp]
	push	eax
	call	_WM_GetParent
	add	esp, 4
	mov	DWORD PTR _hFrame$[ebp], eax
; Line 116
	mov	eax, DWORD PTR _hFrame$[ebp]
	push	eax
	call	_FRAMEWIN_LockH
	add	esp, 4
	mov	DWORD PTR _pFrame$[ebp], eax
; Line 117
	mov	eax, DWORD PTR _pFrame$[ebp]
	mov	cx, WORD PTR [eax+128]
	mov	WORD PTR _Flags$[ebp], cx
; Line 118
	call	_GUI_ALLOC_UnlockH
	mov	DWORD PTR _pFrame$[ebp], 0
; Line 119
	push	0
	call	_GUI_SetColor
	add	esp, 4
; Line 120
	movzx	eax, WORD PTR _Flags$[ebp]
	and	eax, 64					; 00000040H
	je	SHORT $LN2@Draw
; Line 121
	mov	eax, DWORD PTR _hObj$[ebp]
	push	eax
	call	__DrawRestore
	add	esp, 4
; Line 122
	jmp	SHORT $LN3@Draw
$LN2@Draw:
; Line 123
	mov	eax, DWORD PTR _hObj$[ebp]
	push	eax
	call	__DrawMin
	add	esp, 4
$LN3@Draw:
; Line 125
	pop	edi
	pop	esi
	pop	ebx
	add	esp, 228				; 000000e4H
	cmp	ebp, esp
	call	__RTC_CheckEsp
	mov	esp, ebp
	pop	ebp
	ret	0
__Draw	ENDP
_TEXT	ENDS
EXTRN	_WM__GetNextIVR:PROC
EXTRN	_LCD_DrawHLine:PROC
EXTRN	_WM__InitIVRSearch:PROC
EXTRN	_GUI_Context:BYTE
EXTRN	_WM_GetInsideRectEx:PROC
EXTRN	@_RTC_CheckStackVars@8:PROC
; Function compile flags: /Odtp /RTCsu /ZI
;	COMDAT __DrawMin
_TEXT	SEGMENT
_Size$ = -36						; size = 4
_i$ = -24						; size = 4
_r$ = -12						; size = 8
_hObj$ = 8						; size = 4
__DrawMin PROC						; COMDAT
; Line 74
	push	ebp
	mov	ebp, esp
	sub	esp, 232				; 000000e8H
	push	ebx
	push	esi
	push	edi
	lea	edi, DWORD PTR [ebp-232]
	mov	ecx, 58					; 0000003aH
	mov	eax, -858993460				; ccccccccH
	rep stosd
; Line 77
	lea	eax, DWORD PTR _r$[ebp]
	push	eax
	mov	ecx, DWORD PTR _hObj$[ebp]
	push	ecx
	call	_WM_GetInsideRectEx
	add	esp, 8
; Line 78
	movsx	eax, WORD PTR _r$[ebp]
	add	eax, DWORD PTR _GUI_Context+64
	mov	WORD PTR _r$[ebp], ax
	movsx	eax, WORD PTR _r$[ebp+2]
	add	eax, DWORD PTR _GUI_Context+68
	mov	WORD PTR _r$[ebp+2], ax
; Line 79
	movsx	eax, WORD PTR _r$[ebp+4]
	add	eax, DWORD PTR _GUI_Context+64
	mov	WORD PTR _r$[ebp+4], ax
	movsx	eax, WORD PTR _r$[ebp+6]
	add	eax, DWORD PTR _GUI_Context+68
	mov	WORD PTR _r$[ebp+6], ax
; Line 80
	movsx	eax, WORD PTR _r$[ebp+4]
	movsx	ecx, WORD PTR _r$[ebp]
	sub	eax, ecx
	add	eax, 1
	sar	eax, 1
	mov	DWORD PTR _Size$[ebp], eax
; Line 81
	lea	eax, DWORD PTR _r$[ebp]
	push	eax
	call	_WM__InitIVRSearch
	add	esp, 4
	test	eax, eax
	je	SHORT $LN8@DrawMin
$LN6@DrawMin:
; Line 82
	mov	DWORD PTR _i$[ebp], 1
	jmp	SHORT $LN3@DrawMin
$LN2@DrawMin:
	mov	eax, DWORD PTR _i$[ebp]
	add	eax, 1
	mov	DWORD PTR _i$[ebp], eax
$LN3@DrawMin:
	mov	eax, DWORD PTR _i$[ebp]
	cmp	eax, DWORD PTR _Size$[ebp]
	jge	SHORT $LN5@DrawMin
; Line 83
	movsx	eax, WORD PTR _r$[ebp+4]
	sub	eax, DWORD PTR _i$[ebp]
	push	eax
	movsx	ecx, WORD PTR _r$[ebp+6]
	sub	ecx, DWORD PTR _i$[ebp]
	mov	edx, DWORD PTR _Size$[ebp]
	sar	edx, 1
	sub	ecx, edx
	push	ecx
	movsx	eax, WORD PTR _r$[ebp]
	add	eax, DWORD PTR _i$[ebp]
	push	eax
	call	_LCD_DrawHLine
	add	esp, 12					; 0000000cH
; Line 84
	jmp	SHORT $LN2@DrawMin
$LN5@DrawMin:
; Line 85
	call	_WM__GetNextIVR
	test	eax, eax
	jne	SHORT $LN6@DrawMin
$LN8@DrawMin:
; Line 86
	push	edx
	mov	ecx, ebp
	push	eax
	lea	edx, DWORD PTR $LN12@DrawMin
	call	@_RTC_CheckStackVars@8
	pop	eax
	pop	edx
	pop	edi
	pop	esi
	pop	ebx
	add	esp, 232				; 000000e8H
	cmp	ebp, esp
	call	__RTC_CheckEsp
	mov	esp, ebp
	pop	ebp
	ret	0
	npad	3
$LN12@DrawMin:
	DD	1
	DD	$LN11@DrawMin
$LN11@DrawMin:
	DD	-12					; fffffff4H
	DD	8
	DD	$LN10@DrawMin
$LN10@DrawMin:
	DB	114					; 00000072H
	DB	0
__DrawMin ENDP
; Function compile flags: /Odtp /RTCsu /ZI
_TEXT	ENDS
;	COMDAT __DrawRestore
_TEXT	SEGMENT
_Size$ = -36						; size = 4
_i$ = -24						; size = 4
_r$ = -12						; size = 8
_hObj$ = 8						; size = 4
__DrawRestore PROC					; COMDAT
; Line 92
	push	ebp
	mov	ebp, esp
	sub	esp, 232				; 000000e8H
	push	ebx
	push	esi
	push	edi
	lea	edi, DWORD PTR [ebp-232]
	mov	ecx, 58					; 0000003aH
	mov	eax, -858993460				; ccccccccH
	rep stosd
; Line 95
	lea	eax, DWORD PTR _r$[ebp]
	push	eax
	mov	ecx, DWORD PTR _hObj$[ebp]
	push	ecx
	call	_WM_GetInsideRectEx
	add	esp, 8
; Line 96
	movsx	eax, WORD PTR _r$[ebp]
	add	eax, DWORD PTR _GUI_Context+64
	mov	WORD PTR _r$[ebp], ax
	movsx	eax, WORD PTR _r$[ebp+2]
	add	eax, DWORD PTR _GUI_Context+68
	mov	WORD PTR _r$[ebp+2], ax
; Line 97
	movsx	eax, WORD PTR _r$[ebp+4]
	add	eax, DWORD PTR _GUI_Context+64
	mov	WORD PTR _r$[ebp+4], ax
	movsx	eax, WORD PTR _r$[ebp+6]
	add	eax, DWORD PTR _GUI_Context+68
	mov	WORD PTR _r$[ebp+6], ax
; Line 98
	movsx	eax, WORD PTR _r$[ebp+4]
	movsx	ecx, WORD PTR _r$[ebp]
	sub	eax, ecx
	add	eax, 1
	sar	eax, 1
	mov	DWORD PTR _Size$[ebp], eax
; Line 99
	lea	eax, DWORD PTR _r$[ebp]
	push	eax
	call	_WM__InitIVRSearch
	add	esp, 4
	test	eax, eax
	je	SHORT $LN8@DrawRestor
$LN6@DrawRestor:
; Line 100
	mov	DWORD PTR _i$[ebp], 1
	jmp	SHORT $LN3@DrawRestor
$LN2@DrawRestor:
	mov	eax, DWORD PTR _i$[ebp]
	add	eax, 1
	mov	DWORD PTR _i$[ebp], eax
$LN3@DrawRestor:
	mov	eax, DWORD PTR _i$[ebp]
	cmp	eax, DWORD PTR _Size$[ebp]
	jge	SHORT $LN5@DrawRestor
; Line 101
	movsx	eax, WORD PTR _r$[ebp+4]
	sub	eax, DWORD PTR _i$[ebp]
	push	eax
	movsx	ecx, WORD PTR _r$[ebp+2]
	add	ecx, DWORD PTR _i$[ebp]
	mov	edx, DWORD PTR _Size$[ebp]
	sar	edx, 1
	add	ecx, edx
	push	ecx
	movsx	eax, WORD PTR _r$[ebp]
	add	eax, DWORD PTR _i$[ebp]
	push	eax
	call	_LCD_DrawHLine
	add	esp, 12					; 0000000cH
; Line 102
	jmp	SHORT $LN2@DrawRestor
$LN5@DrawRestor:
; Line 103
	call	_WM__GetNextIVR
	test	eax, eax
	jne	SHORT $LN6@DrawRestor
$LN8@DrawRestor:
; Line 104
	push	edx
	mov	ecx, ebp
	push	eax
	lea	edx, DWORD PTR $LN12@DrawRestor
	call	@_RTC_CheckStackVars@8
	pop	eax
	pop	edx
	pop	edi
	pop	esi
	pop	ebx
	add	esp, 232				; 000000e8H
	cmp	ebp, esp
	call	__RTC_CheckEsp
	mov	esp, ebp
	pop	ebp
	ret	0
	npad	3
$LN12@DrawRestor:
	DD	1
	DD	$LN11@DrawRestor
$LN11@DrawRestor:
	DD	-12					; fffffff4H
	DD	8
	DD	$LN10@DrawRestor
$LN10@DrawRestor:
	DB	114					; 00000072H
	DB	0
__DrawRestore ENDP
_TEXT	ENDS
END
