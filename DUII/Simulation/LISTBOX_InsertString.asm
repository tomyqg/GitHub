; Listing generated by Microsoft (R) Optimizing Compiler Version 16.00.40219.01 

	TITLE	C:\DUII\Simulation\GUI\Widget\LISTBOX_InsertString.c
	.686P
	.XMM
	include listing.inc
	.model	flat

INCLUDELIB LIBCMT
INCLUDELIB OLDNAMES

PUBLIC	_LISTBOX_InsertString
EXTRN	_LISTBOX_AddString:PROC
EXTRN	_LISTBOX_InvalidateItem:PROC
EXTRN	_LISTBOX__AddSize:PROC
EXTRN	_strcpy:PROC
EXTRN	_GUI_ALLOC_LockH:PROC
EXTRN	_GUI_ARRAY_InsertItem:PROC
EXTRN	_strlen:PROC
EXTRN	_GUI_ALLOC_UnlockH:PROC
EXTRN	_LISTBOX__GetNumItems:PROC
EXTRN	_LISTBOX_LockH:PROC
EXTRN	__RTC_CheckEsp:PROC
EXTRN	__RTC_Shutdown:PROC
EXTRN	__RTC_InitBase:PROC
;	COMDAT rtc$TMZ
; File c:\duii\simulation\gui\widget\listbox_insertstring.c
rtc$TMZ	SEGMENT
__RTC_Shutdown.rtc$TMZ DD FLAT:__RTC_Shutdown
rtc$TMZ	ENDS
;	COMDAT rtc$IMZ
rtc$IMZ	SEGMENT
__RTC_InitBase.rtc$IMZ DD FLAT:__RTC_InitBase
; Function compile flags: /Odtp /RTCsu /ZI
rtc$IMZ	ENDS
;	COMDAT _LISTBOX_InsertString
_TEXT	SEGMENT
_pItem$11432 = -56					; size = 4
_hItem$11429 = -44					; size = 4
_ItemArray$11426 = -32					; size = 4
_NumItems$11425 = -20					; size = 4
_pObj$11424 = -8					; size = 4
_hObj$ = 8						; size = 4
_s$ = 12						; size = 4
_Index$ = 16						; size = 4
_LISTBOX_InsertString PROC				; COMDAT
; Line 39
	push	ebp
	mov	ebp, esp
	sub	esp, 252				; 000000fcH
	push	ebx
	push	esi
	push	edi
	lea	edi, DWORD PTR [ebp-252]
	mov	ecx, 63					; 0000003fH
	mov	eax, -858993460				; ccccccccH
	rep stosd
; Line 40
	cmp	DWORD PTR _hObj$[ebp], 0
	je	$LN5@LISTBOX_In
	cmp	DWORD PTR _s$[ebp], 0
	je	$LN5@LISTBOX_In
; Line 45
	mov	eax, DWORD PTR _hObj$[ebp]
	push	eax
	call	_LISTBOX_LockH
	add	esp, 4
	mov	DWORD PTR _pObj$11424[ebp], eax
; Line 46
	mov	eax, DWORD PTR _pObj$11424[ebp]
	push	eax
	call	_LISTBOX__GetNumItems
	add	esp, 4
	mov	DWORD PTR _NumItems$11425[ebp], eax
; Line 47
	mov	eax, DWORD PTR _pObj$11424[ebp]
	mov	ecx, DWORD PTR [eax+56]
	mov	DWORD PTR _ItemArray$11426[ebp], ecx
; Line 48
	call	_GUI_ALLOC_UnlockH
	mov	DWORD PTR _pObj$11424[ebp], 0
; Line 49
	mov	eax, DWORD PTR _Index$[ebp]
	cmp	eax, DWORD PTR _NumItems$11425[ebp]
	jae	$LN3@LISTBOX_In
; Line 51
	mov	eax, DWORD PTR _s$[ebp]
	push	eax
	call	_strlen
	add	esp, 4
	add	eax, 12					; 0000000cH
	push	eax
	mov	ecx, DWORD PTR _Index$[ebp]
	push	ecx
	mov	edx, DWORD PTR _ItemArray$11426[ebp]
	push	edx
	call	_GUI_ARRAY_InsertItem
	add	esp, 12					; 0000000cH
	mov	DWORD PTR _hItem$11429[ebp], eax
; Line 52
	cmp	DWORD PTR _hItem$11429[ebp], 0
	je	SHORT $LN2@LISTBOX_In
; Line 53
	mov	eax, DWORD PTR _hItem$11429[ebp]
	push	eax
	call	_GUI_ALLOC_LockH
	add	esp, 4
	mov	DWORD PTR _pItem$11432[ebp], eax
; Line 54
	mov	eax, DWORD PTR _pItem$11432[ebp]
	mov	BYTE PTR [eax+8], 0
; Line 55
	mov	eax, DWORD PTR _pItem$11432[ebp]
	mov	DWORD PTR [eax+4], -1
; Line 56
	mov	eax, DWORD PTR _s$[ebp]
	push	eax
	mov	ecx, DWORD PTR _pItem$11432[ebp]
	add	ecx, 9
	push	ecx
	call	_strcpy
	add	esp, 8
; Line 57
	call	_GUI_ALLOC_UnlockH
	mov	DWORD PTR _pItem$11432[ebp], 0
; Line 58
	mov	eax, DWORD PTR _hObj$[ebp]
	push	eax
	call	_LISTBOX_LockH
	add	esp, 4
	mov	DWORD PTR _pObj$11424[ebp], eax
; Line 59
	mov	eax, DWORD PTR _Index$[ebp]
	push	eax
	mov	ecx, DWORD PTR _pObj$11424[ebp]
	push	ecx
	call	_LISTBOX__AddSize
	add	esp, 8
; Line 60
	call	_GUI_ALLOC_UnlockH
	mov	DWORD PTR _pObj$11424[ebp], 0
; Line 61
	mov	eax, DWORD PTR _Index$[ebp]
	push	eax
	mov	ecx, DWORD PTR _hObj$[ebp]
	push	ecx
	call	_LISTBOX_InvalidateItem
	add	esp, 8
$LN2@LISTBOX_In:
; Line 63
	jmp	SHORT $LN5@LISTBOX_In
$LN3@LISTBOX_In:
; Line 64
	mov	eax, DWORD PTR _s$[ebp]
	push	eax
	mov	ecx, DWORD PTR _hObj$[ebp]
	push	ecx
	call	_LISTBOX_AddString
	add	esp, 8
$LN5@LISTBOX_In:
; Line 68
	pop	edi
	pop	esi
	pop	ebx
	add	esp, 252				; 000000fcH
	cmp	ebp, esp
	call	__RTC_CheckEsp
	mov	esp, ebp
	pop	ebp
	ret	0
_LISTBOX_InsertString ENDP
_TEXT	ENDS
END
