...
static void _DrawBitmap_RLE8(int x0,int y0,int xsize, int ysize, const U8 GUI_UNI_PTR * pPixel, const LCD_LOGPALETTE GUI_UNI_PTR * pLogPal, int xMag, int yMag) {
  ...
  aColorIndex[0] = LCD__GetBkColorIndex();
  aColorIndex[1] = LCD__GetColorIndex();
 ...
 /* Check if we can limit the number of lines due to clipping) */
  ...
  for (; y < ysize; ) {
    ...
    if (Cmd) {
      ...
      Cache.x = xi;
      Cache.y = y;
      LCD__SetColorIndex(pTrans ? *(pTrans+Data) : Data);
      while (Cmd) {
        int xi1 = xi+Cmd;
        if (xi1>=xsize)
          xi1 = xsize;
  ...
  LCD__SetBkColorIndex(aColorIndex[0]);
  LCD__SetColorIndex(aColorIndex[1]);
}
