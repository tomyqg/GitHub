...
void UART_Init (int , unsigned long );
...
void main (	void ) {
	...
	UART_Init(0, ASCIIBAUD);						// enable UART to write to
	UART_Init(1, ASCIIBAUD);						// enable UART to write to
	...
	while (1) {
		u08 port5;
		char *cptr;
		char teststring[] = "hello there, how are you?";
		printf(teststring);
		for (cptr=teststring; *cptr != '\0'; cptr++) {
			SBUF1 = *cptr;
			while (!(SCON1 & 0x02));
		}
	} // while
...
//
void UART_Init (int comport, unsigned long baud)
{
	u08 SFRPAGE_SAVE = SFRPAGE;		// Save Current SFR page
