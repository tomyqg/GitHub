...
#define NUMUBLOXMSGS	3
	...
	// configure protocols and baudrates on ports
//	for (portnum = 0; portnum <= 4; portnum++) {
	for (portnum = 1; portnum <= 1; portnum++) {
		if (2 != portnum) {		// skip COM 2
			// turn off all but Ublox on com, usb, spi ports
			do {	// repeat until we get it right
				i = 0;
		
				packet[UBXPAYLOAD+i++] = portnum;	// 0 port number
				packet[UBXPAYLOAD+i++] = 0;			// 1 reserved
				packet[UBXPAYLOAD+i++] = 0;			// 2 reserved
				packet[UBXPAYLOAD+i++] = 0;			// 3 reserved
				packet[UBXPAYLOAD+i++] = 0xC0;		// 4 110000000	8 bits
				packet[UBXPAYLOAD+i++] = 0x08;		// 5 000000000	1 stop bit, no parity				
				...
				* (s32 *) (packet + UBXPAYLOAD + i) = SWAP32(UBXBAUD);	// 8 57 Kbps
				i += 4;
				packet[UBXPAYLOAD+i++] = 0x00;		// 12 bitmask ublox input enabled
				packet[UBXPAYLOAD+i++] = 0x00;		// 13 bitmask top byte
				packet[UBXPAYLOAD+i++] = 0x00;		// 14 bitmask ublox output enabled
	...
	// For COM 2, turn UBX messages on/off and set the reporting rate
	for (msgcnt = 0; msgcnt < NUMUBLOXMSGS; msgcnt++) {
		do {	// repeat until we get it right
			...
			packet[UBXPAYLOAD+i++] = messagelist[msgcnt].enabled;		// output on uart 2?
