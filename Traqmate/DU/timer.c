	...
	static BOOL inrange;
 	...
 	switch (oper) {
		...
		case NEW_POSITION: {
			...
			// in launch mode, now that we are moving, save the heading shortly after launch
			// limit to small window just in case we are headed due East
			if ((AUTOXGETFINISH == tm_state || AUTOXFINISH == tm_state || DRAGFINISH == tm_state)) {
				if ((0 == lap.startheading) && (lap.timer >= AUTOXSETHDGTIME) && (lap.timer <= (1000 + AUTOXSETHDGTIME))) {
			...
			// don't look for starting line in AutoX or Drag
			if (AUTOXSTAGE == tm_state || DRAGSTAGE == tm_state) return;
...
//					return;
				...
				if (HILLCLIMBGETFINISH == tm_state || AUTOXGETFINISH == tm_state)
					return;

				if (DRAGFINISH == tm_state) {
					// look for distances and record them
// ***** THIS SHOULD BE INTERPOLATED USING ACCELERATION RATES

					// calculate speed in meters/msec
					speed = sqrt((float) eastvel * (float) eastvel + (float) northvel * (float) northvel);

					// 60 foot time = 18.46 meters
					if (0 == dragnumbers.sixtyfoot && lap.distance > 18.46) {
						dragnumbers.sixtyfoot = (u16) (lap.timer) / 10;
#if 0
					...
					// 60 mph = 26.92 mps
					if (0 == dragnumbers.zerosixty && speed > 26.92) {
						dragnumbers.zerosixty = (u16) (lap.timer) / 10;
#if 0
					...
					// 100 mph = 44.70 mps
					if (0 == dragnumbers.zerohundred && speed > 44.70)
						dragnumbers.zerohundred = (u16) (lap.timer / 10);

					// 330 foot time = 101.53 meters
					if (0 == dragnumbers.threethirty && lap.distance > 101.53) {
						dragnumbers.threethirty = (u16) (lap.timer / 10);
#if 0
					...
					// 1/8 mile time = 203.06 meters
					if (0 == dragnumbers.eighthmile && lap.distance > 203.06) {

						dragnumbers.eighthmile = (u16) (lap.timer / 10);
						dragnumbers.eighthtrap = (u16) (speed * 22.356);		// record speed in mph * 10
						...
						if (dragnumbers.quarter) {
						...
						else {		// 1/8 mile run so stop here
							tm_state = DRAGHOLDTIME;
							button_overlay[SEL] = displaydragnums;
							...
							// give them a clue
							Write_Text(3, 4, "Hit SELECT For Data", '-');
						} // else
					} // if

					// 1/4 mile time = 406.12 meters
					if (0 == dragnumbers.quartermile && lap.distance > 406.12) {
						tm_state = DRAGHOLDTIME;
						button_overlay[SEL] = displaydragnums;

						dragnumbers.quartermile = (u16) (lap.timer / 10);
						dragnumbers.quartertrap = (u16) (speed * 22.356);		// record speed in mph * 10
				...
				else if (ABS((s16) lap.currentheading - (s16) seekhdg) < headrange ||
...
//									Write_Timer();
							...
							if (dofinish) {
								...
								if (AUTOXFINISH != tm_state && DRAGFINISH != tm_state) {	// put up all the symbols and get ready for next lap
									// clear the graph
									Write_Icon(0, 23, 122, 8, (u08 *) graphbckgnd, '-');
...
//			tempflash.circuit.track[lap.tracknum].startalt = lap.startpos.alt;
			...
			// jump out if we are not timing
			if (DRIVING(tm_state)) break;

			Clear_Screen();
			if (REV == timingmode) digitoffset = 0;
			else digitoffset = 5;
			Write_Icon(21, 9-digitoffset, 81, 14, (u08 *) timingicon, '-');
			...
			else if (DRAGREADY == tm_state) {
				tm_state = DRAGFINISH;
			} // if
			else {	// not AUTOXGETFINISH or HILLCLIMB
				Write_Screen((u08 *) predictive, '-');
				Write_Icon(111, 23, 8, 8, (u08 *) plussign, '-');
				Write_Icon(3, 23, 8, 8, (u08 *) minussign, '-');
				button_overlay[SEL] = NULL;
